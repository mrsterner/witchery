...
object MutandisLevelDataAttachmentPlatformImpl {
    ...
    fun getTagForBlockPos(level: ServerLevel, pos: BlockPos): TagKey<Block>? {
        val levelData = level.getData(LEVEL_DATA_ATTACHMENT.get()) ?: MutandisLevelAttachment()
    ...
    @JvmStatic
    val LEVEL_DATA_ATTACHMENT: Supplier<AttachmentType<MutandisLevelAttachment>> = ATTACHMENT_TYPES.register(
        "level_data",
        Supplier {
            AttachmentType.builder(Supplier { MutandisLevelAttachment() })
